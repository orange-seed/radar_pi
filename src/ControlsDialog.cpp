...
//---------------------------------------------------------------------------------------
 // m_stayalive_timeout = 0; -edited
 // m_radar_timeout = 0;    -edited
  //m_data_timeout = 0;     -edited
...
time_t m_stayalive_timeout;
//m_control = 0; //edit by sabri, bahari 22/7/2022


bool net_failed = false; // 'false'= network is up, 'true' = network is down
//bool standby_if_time_out = false;
bool autoRadarTX = false;
//
...
void ControlsDialog::OnTransmitButtonClick(wxCommandEvent& event) {
  ...
  //void RadarTxOn()
  {
   ...
   //m_ri->m_state.Update(RADAR_TRANSMIT);
    ...
    // Edited by Bahari, 19/9/2022
    // Remarks: Enable auto retransmitting radar operation
    if (autoRadarTX)
    {
        autoRadarTX = false;
    }
  ...
  {
    // Edited by Bahari, 19/9/2022
    // Remarks: Disable auto retransmitting radar operation upon clicking manually STOP radar
    if (!autoRadarTX)
    {
        autoRadarTX = true;
    }
...
void ControlsDialog::UpdateControlValues(bool refreshAll) {
  ...
  o << m_ri->GetRadarStateText();
    ...
    if (state == RADAR_OFF)
    {
        //Edited by Sabri, enhanced by Bahari, 22/7/2022
        // Set network failed flag, declared at line 197
        net_failed = true; //netwok is down
        // For safety, make sure this flag are clear during network failed
        if (autoRadarTX)
        {
            autoRadarTX = false;
        }
        //standby_if_time_out = true;

        DisableRadarControls();

        //edited by Sabri Azman, 21/7/2022
        // added line 2097-2099
    ...
    {
        /*
        // Previously network failed
        if (net_failed)
        {
            m_ri->RequestRadarState(RADAR_TRANSMIT);
            net_failed = false; // now network is restored
            standby_if_time_out = false;
        }
        else if (!standby_if_time_out)
        {
            m_ri->RequestRadarState(RADAR_TRANSMIT);
            standby_if_time_out = false;
        }
        */
        // Edited by Bahari, 19/9/2022
        // Remarks: Radar will be auto re transmit during STANBY mode and RADAR OFF (network failure)
        if ((!autoRadarTX) || (net_failed))
        {
            // Clear network failed flag
            if (net_failed)
            {
                net_failed = false;
            }
