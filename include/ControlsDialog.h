...
#undef HAVE_CONTROL
//#include "RadarInfo.h" // edit by sabri
//#include "RadarControl.h" // edit by sabri
...
public:
    ControlsDialog()
    {
        ...
        m_from_sizer
            = 0; // If on edit control, this is where the button is from
        m_current_sizer = 0;
    ...
    void HideTemporarily(); // When a second dialog (which is not a child class)
                            // takes over -- aka GuardZone
...
protected:
    ...
    wxBoxSizer*
        m_from_sizer; // If on edit control, this is where the button is from
    ...
    RadarControlButton* m_accent_light_button;
...
public:
    ...
    {
        Create(parent, id, label, wxDefaultPosition, buttonSize, 0,
            wxDefaultValidator, label);
...
public:
    ...
    DynamicStaticText(wxWindow* parent, wxWindowID id, const wxString& label,
        const wxPoint& pos = wxDefaultPosition,
...
public:
    ...
    {
        Create(parent, id, label + wxT("\n"), wxDefaultPosition, g_buttonSize,
            0, wxDefaultValidator, label);
    ...
    virtual bool
    ToggleValue(); // Returns true if value was toggled to next value
    virtual bool
    ToggleState(); // Returns desired new state for Auto/Off button show.
